...
def notify(*messages, **options):
    ...
    xbmc.executebuiltin('Notification("%s","%s",%d,"%s")' % (xbmcaddon.Addon().getAddonInfo('name'), str(' ').join(messages), time, image))
...
def log(*messages, **options):
    ...
    if level:
        ...
        xbmc.log('%s: %s(%d): %s: %s' % (addon.getAddonInfo('id'), os.path.basename(frame.f_code.co_filename), frame.f_lineno, frame.f_code.co_name, ' '.join(messages)), level)
